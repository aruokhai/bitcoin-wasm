...
use crate::{json_schemas::generated::DRAFT_07_JSON_SCHEMA, request::{get_scheme, request, SchemeProps}, };
...
impl LocalSchemaResolver {
    ...
    fn normalize_url(url: &str) -> String {
        let normalized_url = url.clone().split("#").collect::<Vec<&str>>()[0];
...
impl SchemaResolver for LocalSchemaResolver {
    fn resolve(
        ...
        if let Some(schema) = self.schemas.get(&LocalSchemaResolver::normalize_url(url.as_str())) {
            Ok(std::sync::Arc::new(schema.clone()))
        } else {
            let SchemeProps { url, url_scheme, url_path } = get_scheme(url.as_str());
            let response = request(Method::Get, url_scheme, &url, &url_path, None, None, None, None, None);
            match response{
                Ok(response) => {
                    let schema: serde_json::Value = serde_json::from_slice(&response.body).map_err(|err| {
