...
use crate::{
    get_utc_now, json::{FromJson, ToJson}, json_schemas::generated::{
        MESSAGE_JSON_SCHEMA, RFQ_DATA_JSON_SCHEMA, RFQ_PRIVATE_DATA_JSON_SCHEMA,
    }, messages::MessageError, resources::offering::Offering, DEFAULT_PROTOCOL_VERSION
};
...
fn generate_random_salt() -> String {
    let mut salt = [0u8; 16];
    let random_data = random::random::get_random_bytes(16);
    general_purpose::URL_SAFE_NO_PAD.encode(random_data)
}
