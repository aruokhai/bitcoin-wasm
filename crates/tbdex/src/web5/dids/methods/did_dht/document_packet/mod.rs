use crate::web5::crypto::{dsa::DsaError, jwk::JwkError};
use crate::web5::dids::data_model::document::Document;
use crate::web5::dids::data_model::{service::Service, verification_method::VerificationMethod};
...
fn reconstitute_verification_relationship(
    indices: &[u32],
    idx_to_vm_id: &BTreeMap<u32, String>,
...
impl Document {
    pub fn to_packet(&self) -> Result<Packet, DocumentPacketError> {
        ...
        let mut vm_id_to_idx: BTreeMap<String, u32> = BTreeMap::new();
    ...
    pub fn from_packet(packet: &Packet) -> Result<Document, DocumentPacketError> {
        ...
        let mut idx_to_vm_id: BTreeMap<u32, String> = BTreeMap::new();
...
#[cfg(test)]
mod tests {
    use crate::web5::crypto::dsa::ed25519::{self, Ed25519Generator};
