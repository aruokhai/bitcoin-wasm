...
impl BTreeBuilder {
    ...
    pub fn path(mut self, path: Descriptor) -> BTreeBuilder {
        self.path = path;
        self
    }
...
impl BTree {
    ...
    fn search_node(&mut self, node: Node, search: &str) -> Result<KeyValuePair, Error> {
        match node.node_type {
            NodeType::Internal(children, keys) => {
                let idx = keys
                    .binary_search(&search.to_string())
                    .unwrap_or_else(|x| x);
    ...
    fn delete_key_from_subtree(
        ...
        match &mut node.node_type {
            NodeType::Leaf(ref mut pairs) => {
                let key_idx = pairs
                    .binary_search_by_key(&key, |kv| kv.key.clone())
...
mod tests {
    ...
    fn delete_works() -> Result<(), Error> {
        ...
        btree.delete("d".to_string())?;
        res = btree.search("d".to_string());
        assert!(matches!(res, Err(Error::KeyNotFound)));

        btree.delete("e".to_string())?;
        res = btree.search("e".to_string());
        assert!(matches!(res, Err(Error::KeyNotFound)));

        btree.delete("f".to_string())?;
